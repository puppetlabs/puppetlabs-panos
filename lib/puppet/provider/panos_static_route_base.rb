...
class Puppet::Provider::PanosStaticRouteBase < Puppet::Provider::PanosProvider
  ...
  def get_sr_version(context)
    sr_version = context.type.definition[:name].include? 'ipv6' unless context.type.definition[:name].nil?
    sr_version = context[:name].include? 'ipv6' if context.type.definition[:name].nil?
    sr_version = sr_version ? 'ipv6' : 'ip'
    sr_version
  end
  ...
  def validate_should(should)
    ...
    raise Puppet::ResourceError, 'BFD requires a nexthop ip address to be specified.' if should[:bfd_profile] != 'None' && should[:nexthop_type] !~ %r{^ip(?:v6)?-address$}
  end
  ...
  def get(context)
    ...
    config.elements.collect('/response/result/entry') do |entry| # rubocop:disable Style/CollectionMethods
      ...
      config.elements.collect("/response/result/entry[@name='#{vr_name}']/routing-table/#{get_sr_version(context)}/static-route/entry") do |static_route_entry|
        result = {}
  ...
  def create(context, _name, should)
    context.type.definition[:base_xpath] = "/config/devices/entry/network/virtual-router/entry[@name='#{should[:vr_name]}']/routing-table/#{get_sr_version(context)}/static-route"
  ...
  def delete(context, name, vr_name)
    context.device.delete_config(context.type.definition[:base_xpath] + "/entry[@name='#{vr_name}']/routing-table/#{get_sr_version(context)}/static-route/entry[@name='#{name}']")
  end
end
