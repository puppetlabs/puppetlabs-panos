...
RSpec.describe 'the panos_static_route type' do
  it 'loads' do
    expect(Puppet::Type.type(:panos_static_route)).not_to be_nil
  end

  it 'has a base_xpath' do
    expect(Puppet::Type.type(:panos_static_route).context.type.definition).to have_key :base_xpath
  end
  ...
  context 'when `route` exceeds 31 characters' do
    ...
    it 'throws an error' do
      ...
      expect {
        Puppet::Type.type(:panos_static_route).new(
          name: route,
          vr_name: 'vr name',
        )
